...
var SupplierFields = struct {
	Id                       string
	ContractId               string
	FullName                 string
	Status                   string
	TaxId                    string
	ServingCountries         string
...
var SupplierMeta = &daog.TableMeta[Supplier]{
	Table: "supplier",
	Columns: []string{
		"id",
		"contract_id",
	...
	AutoColumn: "id",
	LookupFieldFunc: func(columnName string, ins *Supplier, point bool) any {
		if "id" == columnName {
			if point {
				return &ins.Id
			}
		...
		if "serving_countries" == columnName {
			if point {
				return &ins.ServingCountries
			}
			return ins.ServingCountries
		}
...
type Supplier struct {
	...
	FullName                 string
	Status                   ttypes.NilableString
	TaxId                    ttypes.NilableString
	ServingCountries         ttypes.NilableString
...
func (s *Supplier) ToDomainInfo() *model.DomainInfo {

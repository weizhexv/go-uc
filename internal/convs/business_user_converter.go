...
type BusinessUserConv struct {
	uu *model.UpsertUser
}

func NewBusinessUserConv(uu *model.UpsertUser) *BusinessUserConv {
	return &BusinessUserConv{uu}
}

func (c *BusinessUserConv) ToPO(ctx *dgctx.DgContext) *dal.BusinessUser {
	var role int
	if c.uu.Role != nil {
		role = c.uu.Role.Id
	}
	return &dal.BusinessUser{
		Uid:        c.uu.Uid,
		CustomerId: c.uu.DomainId,
		RoleId:     role,
		Name:       c.uu.Name,
...
func (c *BusinessUserConv) UpdatePO(ctx *dgctx.DgContext, po *dal.BusinessUser) error {
	if po == nil {
